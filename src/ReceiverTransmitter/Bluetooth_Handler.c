...
*/
        ...
        if (Bluetooth_Forward == 1) {
            ...
            char GPSDATA[100];
            sprintf(GPSDATA, "MyID:%lf,Latitude:%lf,Longitude:%lf,Altitude:%lf,Speed:%lf,Direction:%lf,",
                    MyID_Bluetooth, GPS->latitude, GPS->longitude, GPS->altitude, GPS->speed, GPS->course);
            // printf("33\n\r");
